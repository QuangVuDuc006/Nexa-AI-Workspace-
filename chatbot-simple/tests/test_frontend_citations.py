...
def test_frontend_citation_renderer_converts_known_marker_to_pill_parts(tmp_path):
...
def test_frontend_citation_renderer_builds_clickable_fallback_href(tmp_path):
    result = run_citation_module(
        """
        import { citationHref } from "__MODULE__";
        console.log(JSON.stringify({
          href: citationHref({
            id: "2",
    ...
    assert result["href"] == "/api/documents/doc-source-2/content#page=14"
